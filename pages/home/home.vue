<template>
	<view>
		...
		<!-- 自提和代取按钮-->
		<view class="buttons">
			<button class="button" @click="handleButtonClick(1)">
				<img src="/static/img/btn1.png" alt="Button 1 Icon" class="button-icon" />
				<span class="button-text">自提</span>
			</button>
			<button class="button" @click="handleButtonClick(2)">
				<img src="/static/img/btn2.png" alt="Button 2 Icon" class="button-icon" />
				<span class="button-text">代取</span>
			</button>
		</view>
		<!-- 功能栏 -->
		<view class="toolbar">
		...
		<!-- 最近订单-->
		<view class="recentOrder">
			<!-- 顶部导航 -->
			<view class="title">最近订单
				<view class="showMore" @tap="toPage('../../subpkg/user/order_list/order_list')" style="color: blue;">全部</view>
			</view>
	...
	export default {
		data() {
			return {
				...
				mytoolbarList: [{
						url: '../../subpkg/user/keep/keep',
						text: '我的收藏',
						img: '/static/img/user/point.png'
					},
					{
						url: '../../subpkg/user/coupon/coupon',
						text: '优惠券',
						img: '/static/img/user/quan.png'
					},
					{
						url: '../../subpkg/user/address/address',
						text: '收货地址',
		...
		methods: {
			...
			toPage(url){
				if(!url){
					uni.showToast({title: '未包含此页面',icon:"none"});return;
				}
				uni.navigateTo({
					url:url
				})
...
<style lang="scss">
	...
	.buttons {
		...
		.button {
			&:nth-child(1) {
				background-color: #00aaff; // 浅蓝色
				color: #fff
			}

			&:nth-child(2) {
				background-color: #55aa7f; // 浅绿色
				color: #fff
			}
